...
struct trie_node {
    ...
    pthread_mutex_t nodetex; /* Node mutex */
};
...
struct trie_node * new_leaf (const char *string, size_t strlen, int32_t ip4_address) {
    ...
    assert(pthread_mutex_init(&new_node->nodetex, NULL) == 0);
...
_search (struct trie_node *node, const char *string, size_t strlen) {
    ...
    if (node == NULL) return NULL;    // Don't need to unlock because null
    ...
    if (cmp == 0) {
        ...
        if (node->strlen > keylen) {
            ...
            // Lock the child (in the basement) (just kidding) (or am I?)
        ...
        // Node's key is "less" than the search key
        if (cmp < 0) {
            // Lock the child
            if (node->next != NULL) {
...
int _insert (const char *string, size_t strlen, int32_t ip4_address, 
    ...
    if (cmp == 0) {
        ...
        if (overlap) {
            ...
            assert(pthread_mutex_lock(&new_node->nodetex) == 0);
